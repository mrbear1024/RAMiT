...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert CBCT pickle files to .npy format using parallel workers.",
    )
    parser.add_argument(
        "--in-dir",
...
def _build_out_path(
    pkl_path: Path, in_dir: Path, raw_root: Path, out_root: Path
) -> Path:
    rel_path = pkl_path.relative_to(raw_root or in_dir)
    rel_path = _map_out_rel_path(rel_path)
    return (out_root / rel_path).with_suffix(".npy")


def _convert_one(pkl_path: Path, out_path: Path) -> None:
...
def main() -> int:
    ...
    for pkl_path in pkl_files:
        out_path = _build_out_path(pkl_path, in_dir, raw_root, out_root)
        ...
        if args.dry_run:
            ...
            log_progress()
            continue
        tasks.append((pkl_path, out_path))
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = [
            executor.submit(_convert_one, pkl_path, out_path)
            for pkl_path, out_path in tasks
        ]
    ...
    if errors:
        raise SystemExit(f"Conversion failed: {errors[0]}")
